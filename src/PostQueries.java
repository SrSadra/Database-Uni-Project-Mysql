public class PostQueries {
    ...
    public static final String GET_POST_LIKES_COUNT = "SELECT count(distinct profile_id) as counts FROM likes where post_id = ?";
    ...
    "VALUES (? , ?, ?, ?,? ,?);";
    public static final String CREATE_POST_LIKE = "INSERT INTO likes " +
    ...
    public static final String GET_USER_FRIENDS_LIKED_POSTS = "SELECT p.*,pro.username FROM post p INNER JOIN likes l ON p.post_id = l.post_id INNER JOIN connection c ON l.profile_id = c.profile_id INNER JOIN profile pro ON pro.id = p.profile_id where c.friend_id = ? ORDER BY p.time";
